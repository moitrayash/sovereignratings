...
- **Verra carbon-project analysis** (added 2026-05-03 per user) — apply the M1/M2/M4 + shadow methodology framework to voluntary-carbon-market projects. Verra registry pulls cleanly via `POST https://registry.verra.org/uiapi/resource/resource/search` with body `{"program":"VCS","resourceClass":"project"}`, returns 4,960 projects × 15 fields (resourceIdentifier, resourceName, proponent, protocolCategories, protocolSubCategories, protocols, resourceStatus, country, region, estAnnualEmissionReductions, version, projectRegistrationDate, creditingPeriodStartDate, creditingPeriodEndDate, createDate). Snapshot already saved to `verra_projects.csv` in the working folder. Possible directions: (a) compute relative ratings (M1/M2/M4) over peer sets defined by methodology + country + AFOLU sub-category, (b) build a Sylvera/Abatable/BeZero comparison page, (c) shadow-rate each project from registry-observable features (country, methodology, vintage) and flag deviations. Gold Standard registry is auth-walled; ACR / CAR / ART are scrape-able but APX-hosted ASP forms (more involved). Aggregator option: Allied Offsets (paid).
