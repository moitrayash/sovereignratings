...
// ── v90: hover-tooltip formula popups for M1/M2/M3/M4/ΔM1 ─────────
// Yash asked for: hover over any "M1" (or M2/M3/M4/ΔM1) token
// anywhere in body text and get a small popup showing just the formula.
// Implemented as a one-time DOM walk on DOMContentLoaded plus a CSS
// rule that styles a dotted-underline span and renders the formula in a
// tooltip on :hover. Skips chart SVGs, code blocks, form controls.
(function scrFormulaTooltips(){
  if (typeof window === 'undefined' || typeof document === 'undefined') return;
  if (window.SCR_FORMULAS_INSTALLED) return;
  window.SCR_FORMULAS_INSTALLED = true;

  window.SCR_FORMULAS = {
    'ΔM1': 'M1ₜ − M1ₜ₋₁  (year-on-year change in M1)',
    'M1': 'Score² ÷ Group LOO mean',
    'M2': 'rank within peer group',
    'M3': 'Score² ÷ lagged Group LOO mean',
    'M4': 'within-year percentile (0–1)'
  };
  // Match longer tokens first so ΔM1 is consumed before M1 inside it.
  var TOKENS = ['ΔM1', 'M1', 'M2', 'M3', 'M4'];

  // CSS injection - one <style> tag at the top of <head>.
  function installStyle(){
    if (document.getElementById('scr-formula-pop-style')) return;
    var s = document.createElement('style');
    s.id = 'scr-formula-pop-style';
    s.textContent = [
      '.formula-pop { cursor: help; border-bottom: 1px dotted currentColor; position: relative; display: inline; }',
      '.formula-pop:hover::after {',
      ...
      '  pointer-events: none;',
      '}',
      // Dark-mode swap: invert tooltip bg/fg when body has .dark.
      'body.dark .formula-pop:hover::after { background: var(--bg-card, #ececec); color: var(--ink, #1a1a1a); }',
      'body.dark .formula-pop:hover::before { border-top-color: var(--bg-card, #ececec); }'
    ].join('\n');
    document.head.appendChild(s);
  }

  var SKIP_TAGS = { SCRIPT:1, STYLE:1, CODE:1, PRE:1, TEXTAREA:1, INPUT:1,
    OPTION:1, SELECT:1, BUTTON:1, SVG:1, G:1, TEXT:1, TSPAN:1, DEFS:1,
    PATH:1, RECT:1, CIRCLE:1, LINE:1, POLYGON:1, POLYLINE:1, MASK:1,
    CLIPPATH:1, USE:1, IMAGE:1, FOREIGNOBJECT:1 };
  ...
  function processTextNode(node){
    ...
    if (!t) return;
    // Quick skip: no token char present, no work.
    if (t.indexOf('M') < 0 && t.indexOf('Δ') < 0) return;
    ...
    while (i < t.length) {
      ...
      if (matched) {
        ...
        span.className = 'formula-pop';
        span.setAttribute('data-formula', window.SCR_FORMULAS[matched]);
  ...
  function walk(node){
    if (!node) return;
    if (node.nodeType === 3) { processTextNode(node); return; }
    if (node.nodeType !== 1) return;
    var tag = node.tagName ? node.tagName.toUpperCase() : '';
    if (SKIP_TAGS[tag]) return;
    if (node.classList && node.classList.contains('formula-pop')) return;
    // Snapshot children since we mutate during walk.
    var kids = [];
  ...
  function go(){
    try { installStyle(); } catch(e) { /* ignore */ }
    try { walk(document.body); } catch(e) { console.warn('formula tooltip walk failed:', e); }
  }
